...
path_to_TOP_instances = os.path.join(os.getcwd(), "TOP")
path_to_CEDTOPADS_instances = os.path.join(os.getcwd(), "CEDTOPADS")
path_to_CEDOPADS_instances = os.path.join(os.getcwd(), "CEDOPADS")
...
for idx, file_id in enumerate(["p4.2.a.txt", "p5.2.a.txt", "p6.2.a.txt", "p7.2.a.txt"]):
    ...
    # Save files
    for rho_idx, rho in enumerate([k/2 * sensing_radii[idx]["r_min"] for k in range(1, 5)]):
        for eta_idx, eta in enumerate([k / 4 * np.pi for k in range(1, 5)]):
